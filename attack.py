...
def expected_acc(models, prob, x, y, reduction='mean'):
    acc = 0
...
def arc_attack(models, x_clean, label, prob, epsilon = 0.3, step_size = 2 / 255.0, attack_iter=10, rand=False, num_classes=10, g=1, restarts=1, use_rand_loop=True):
    for net in models:
        net.eval()
    M = len(prob)
    upper_limit = 1
    ...
    lower_limit = 0
    topk = g + 1
    x_clean = x_clean.view(-1, 784)  # 假设每个样本有 784 个特征
    label = label.view(-1)           # 确保 label 的形状为一维
    ...
    for _ in range(restarts):
        ...
        loss_min = expected_acc(models,prob,x_clean,label,reduction='none')
        if rand:
            ...
            for i in I:
                ...
                for j in range(1,topk):
                    ...
                    with torch.no_grad():
                        ...
                        g_j = -torch.sign(w_j)
                        g_2_j = -w_j/(del_norm.view(-1,1)+1e-10)
                        #print("g_2 shape:", g_2.shape)
                        #print("g_2_j shape:", g_2_j.shape)
                ...
                delta_local_hat = delta_local + beta.view(-1,1)*g
                delta_local_hat = beta_m*delta_local_hat/(torch.norm(delta_local_hat.view(w_j.shape[0],-1),p=float('inf'),dim=1).view(-1,1)+1e-10)

                d = torch.clamp(delta_min + delta_local_hat, min=-epsilon, max=epsilon)

                d = clamp(d, lower_limit - x_clean, upper_limit - x_clean)

                new_loss =  expected_acc(models,prob,x_clean+d,label,reduction='none')
...
def apgd_attack(model, x_clean, label, epsilon = 8 / 255.0, step_size = 2 / 255.0, attack_iter=10, other_weight=0., rand=True, inv=False, num_classes=10):
    ...
    for _ in range(attack_iter):
        ...
        with torch.enable_grad():
            pred = torch.log(F.softmax(model(x), dim=1))

            loss = criterion(pred, label)
            other_pred = -pred
            one_hot_targets = F.one_hot(label, num_classes=10).float()
            other_advloss = - F.log_softmax(other_pred, dim=1) * (1 - one_hot_targets)
            other_advloss = other_advloss.sum(dim=1).mean() / (10 - 1)
